...
Options for install:
  ...
  -c, --clean               Clean the build cache first
      ...
      --skip-updateicons    Skip Updating Icon packs if they already exists
...
para=$(getopt \
  -o hfFk:cs \
  -l help,force,firstrun,fontset:,clean,skip-allgreeting,skip-alldeps,skip-allsetups,skip-allfiles,skip-updateicons,ignore-outdate,skip-sysupdate,skip-plasmaintg,skip-backup,skip-quickshell,skip-fish,skip-hyprland,skip-hyprland-entry,skip-fontconfig,skip-miscconf,core,exp-files,via-nix \
  -n "$0" -- "$@")
...
while true ; do
  case "$1" in
    ...
    --skip-updateicons) SKIP_UPDATE_ICONS=true;shift;;
